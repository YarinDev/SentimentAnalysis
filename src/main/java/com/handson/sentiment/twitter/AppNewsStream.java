...
public class AppNewsStream {
    ...
    // this method is used to fetch the news from the news api and then filter the news based on the keyword
    public Flux<String> fetch(String keyword) throws InterruptedException {
        ...
        // emitterProcessor is used to emit the flux of news articles sentence by sentence
        emitterProcessor = EmitterProcessor.create();
        emitterProcessor.map(x -> x);
        scheduler = Executors.newScheduledThreadPool(1);
        // setting the scheduler to emit the news articles every 300 milliseconds for steady flow of news articles
    ...
    // runnable (triggered by the scheduler) is used to check if the article is not null and then emit the article to the emitterProcessor
    Runnable runnable = () -> {
